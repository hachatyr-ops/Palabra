...
const App = () => {
  ...
  useEffect(() => { localStorage.setItem('palabra_words', JSON.stringify(words)); }, [words]);
  useEffect(() => { 
  ...
  useEffect(() => {
    if (editConfirmId) {
      const timer = setTimeout(() => setEditConfirmId(null), 3000);
      return () => clearTimeout(timer);
    }
  }, [editConfirmId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const word: Word = { id: Date.now().toString(), spanish: newSp, russian: newRu, addedAt: Date.now() };
      setWords([word, ...words]);
      finalId = word.id;
    }
  ...
  const handleAutoTranslate = async () => {
    ...
    // BUG FIX: Always translate to Russian, even if UI is English
  ...
  const handleRandomWord = async () => {
    ...
    const result = await getRandomWord(lang, alphabetFilter);
  ...
  const handleExport = () => {
    const textData = words.map(w => `${w.spanish}, ${w.russian}`).join('\n');
    const blob = new Blob([textData], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `palabra_words.txt`;
    link.click();
  };
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      lines.forEach(line => {
        ...
        if (parts.length >= 2) {
          const sp = parts[0].trim();
          const ru = parts[1].trim();
          if (sp && ru) {
            newWords.push({ id: (Date.now() + Math.random()).toString(), spanish: sp, russian: ru, addedAt: Date.now() });
          }
        }
      });
  ...
  const scrollAlphabet = (direction: 'left' | 'right') => {
    if (alphabetScrollRef.current) {
      ...
      alphabetScrollRef.current.scrollBy({
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] font-sans text-slate-900 pb-32">
      ...
      <main className="max-w-xl mx-auto px-4">
        {/* STICKY NAVIGATION BAR - Locked to Header height with border-b */}
          ...
          <div className="bg-slate-100/50 p-1.5 rounded-[22px] flex shadow-inner-sm">
            {(['home', 'dictionary', 'quiz'] as const).map(v => (
              <button 
                ...
                className={`flex-1 py-3.5 rounded-[18px] font-black text-[11px] uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 ${view === v ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 active:bg-slate-200/50'}`}
              >
            ...
            <form onSubmit={handleSubmit} className={`bg-white p-5 rounded-[32px] shadow-2xl shadow-blue-900/5 border transition-all space-y-3 ${editingId ? 'border-orange-200 ring-4 ring-orange-50' : 'border-slate-50'}`}>
              <div className="flex items-center justify-between pb-1">
                <h3 className="font-black text-slate-800 text-xs uppercase tracking-widest flex items-center gap-2">
                  {editingId ? t.editWord : t.addWord}
                </h3>
                {!editingId && (
                  <button 
                    type="button" 
                    ...
                    className="text-[10px] font-black uppercase text-white bg-indigo-500 hover:bg-indigo-600 px-5 py-2 rounded-xl transition-all disabled:opacity-50 active:scale-95 shadow-md shadow-indigo-100"
                  >
                    {isGettingRandom ? '...' : (lang === 'ru' ? 'РАНДОМ' : 'RANDOM')}
                  </button>
                )}
              </div>
              ...
              <div className="space-y-2.5">
                <div className="space-y-1">
                  ...
                  <input 
                    className="w-full py-2.5 px-4 bg-slate-50 border-2 border-slate-300 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 focus:bg-white focus:border-blue-400 transition-all text-lg font-medium placeholder:text-slate-300" 
                    placeholder={t.placeholderSp} 
                    value={newSp} 
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center px-1">
                    ...
                    <button 
                      ...
                      disabled={isTranslating || !newSp.trim()} 
                      className="text-[10px] font-black uppercase bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 transition-colors disabled:bg-slate-100 disabled:text-slate-300 shadow-md shadow-blue-100"
                    >
                  ...
                  <div key={w.id} className="bg-white py-3 px-4 rounded-[22px] border border-slate-50 shadow-sm flex justify-between items-center transition-all animate-in fade-in slide-in-from-bottom-1 overflow-hidden">
                    ...
                    <div className="flex items-center gap-1">
                      <button 
                        ...
                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${editConfirmId === w.id ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-300 active:bg-blue-50 active:text-blue-500'}`}
                      >
                        <i className={`fas fa-${editConfirmId === w.id ? 'check' : 'pencil-alt'} text-xs`}></i>
                      </button>
                      <button 
                        type="button" 
                        onClick={(e) => handleDelete(w.id, e)} 
                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${deleteConfirmId === w.id ? 'bg-red-500 text-white scale-110 shadow-lg shadow-red-200 animate-pulse' : 'text-slate-300 active:bg-red-50 active:text-red-500'}`}
                      >
                        <i className={`fas fa-${deleteConfirmId === w.id ? 'check' : 'trash-alt'} text-xs`}></i>
                      </button>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            <div className="relative group -mx-4 overflow-hidden">
              <button 
                ...
                className="absolute left-0 top-0 z-20 w-10 h-[42px] bg-[#FDFDFF] border-r border-slate-100 flex items-center justify-center text-slate-400 hover:text-blue-500 transition-colors shadow-sm"
              >
                <i className="fas fa-chevron-left text-[10px]"></i>
              </button>
              <div 
                ref={alphabetScrollRef} 
                className="w-full overflow-x-auto pb-1 no-scrollbar px-10 scroll-smooth"
              >
                <div className="flex gap-1.5 py-px">
                  {SPANISH_ALPHABET.map(letter => (
                    <button 
                      ...
                      className={`min-w-[42px] h-[42px] rounded-2xl font-black text-xs transition-all border flex items-center justify-center ${alphabetFilter === letter ? 'bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-100' : 'bg-white text-slate-400 border-slate-100 active:bg-slate-50'}`}
                    >
                      {letter === 'All' ? <i className="fas fa-layer-group text-[10px]"></i> : letter}
                    </button>
                  ))}
                </div>
              </div>
              <button 
                ...
                className="absolute right-0 top-0 z-20 w-10 h-[42px] bg-[#FDFDFF] border-l border-slate-100 flex items-center justify-center text-slate-400 hover:text-blue-500 transition-colors shadow-sm"
              >
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <input 
                  className="w-full pl-10 pr-5 py-2.5 bg-slate-50 border-2 border-slate-300 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 focus:bg-white focus:border-blue-400 transition-all text-sm font-medium shadow-sm" 
              ...
              {filteredAndSortedWords.slice(0, visibleItemsCount).map(w => (
                <div key={w.id} className="bg-white py-3 px-4 rounded-[22px] border border-slate-50 shadow-sm flex justify-between items-center transition-all group overflow-hidden">
                  ...
                  <div className="flex items-center gap-1 relative z-30">
                    ...
                    {/* Fixed typo in className template strings */}
                    <button type="button" onClick={(e) => handleDelete(w.id, e)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${deleteConfirmId === w.id ? 'bg-red-500 text-white scale-110 shadow-lg shadow-red-200 animate-pulse' : 'text-slate-300 active:bg-red-50 active:text-red-500'}`}>
                      <i className={`fas fa-${deleteConfirmId === w.id ? 'check' : 'trash-alt'} text-xs`}></i>
                    </button>
                  </div>
                </div>
          ...
          <div className="animate-in slide-in-from-right duration-500 pt-2">
            {isQuizFinished ? (
              ...
              <div className="bg-white px-4 py-4 rounded-[32px] border border-slate-50 shadow-2xl shadow-blue-900/10 text-center space-y-4 relative overflow-hidden flex flex-col justify-between">
                ...
                <div className="flex justify-between items-center px-1">
                  <p className="text-[10px] font-black uppercase text-slate-300">{quizIndex + 1} / {quizWords.length}</p>
                </div>
                ...
                <div className="space-y-4 py-2">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <button 
                      type="button" 
                      onClick={() => playWordAudio(quizWords[quizIndex].spanish)} 
                      disabled={isPlayingAudio} 
                      className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg transition-all flex-shrink-0 ${isPlayingAudio ? 'bg-blue-100 text-blue-400 animate-pulse' : 'bg-blue-600 text-white active:scale-90 shadow-md shadow-blue-100'}`}
                    >
                ...
                {aiHint && (
                  <div className="p-3 bg-yellow-50/50 rounded-xl border border-yellow-100 text-[10px] text-yellow-800 italic leading-tight animate-in fade-in slide-in-from-bottom-1">
                    {aiHint}
                  </div>
                )}
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-6 py-4 pb-8 flex items-center justify-between z-50">
        {view === 'home' && <div className="h-2 w-full"></div>}
        {view === 'dictionary' && (
          <div className="grid grid-cols-2 w-full gap-2 h-12">
            <button 
              type="button" 
              onClick={() => fileInputRef.current?.click()} 
              className="h-12 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center active:bg-blue-700 transition-all shadow-lg shadow-blue-100"
            >
              {t.importBtn}
            </button>
            <button 
              ...
              className="h-12 bg-green-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center active:bg-green-700 transition-all shadow-lg shadow-green-100"
            >
              {t.exportBtn}
            </button>
        ...
        {view === 'quiz' && !isQuizFinished && (
          <div className="grid grid-cols-3 w-full gap-2 h-12">
            <button 
              type="button" 
              onClick={() => setIsQuizFinished(true)} 
              className="h-12 bg-red-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center active:bg-red-600 transition-all shadow-lg shadow-red-100"
            >
              {t.stop}
            </button>
            ...
            <button 
              type="button" 
              onClick={handleAiHintRequest} 
              disabled={quizResult !== 'wrong' || isAiLoading} 
              className={`h-12 font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center transition-all ${quizResult === 'wrong' ? 'bg-blue-600 text-white shadow-lg shadow-blue-100 active:bg-blue-700' : 'bg-slate-50 text-slate-200'}`}
            >
              {isAiLoading ? '...' : t.aiHelpBtn}
            </button>
            ...
            {quizResult === 'wrong' ? (
              <button 
                ...
                className="h-12 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center active:bg-blue-700 shadow-lg shadow-blue-100 animate-in fade-in"
              >
                {t.nextBtn}
              </button>
            ) : (
